namespace Domain
{
    ...
    public class CacheSyncronizeService
    {
        ...
        static async Task IncreaseCacheVersion(Type modifiedObjectType)
        {
            ...
            if (modifiedObjectType == typeof(GeneralSettings)) return;
            ...
            GeneralSettings.Current.CacheVersion++;

            // Ensure the database is universally incremented, even if my version was already stale.
            await Database.GetAccess<GeneralSettings>().ExecuteNonQuery("UPDATE dbo.GeneralSettings set CacheVersion = CacheVersion + 1");
        }
        ...
        {
            var cacheVersion = await Database.GetAccess<GeneralSettings>()
                .ExecuteScalar<int>("SELECT TOP 1 CacheVersion FROM dbo.GeneralSettings");

            if (cacheVersion != GeneralSettings.Current.CacheVersion)
            {
